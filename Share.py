...
# ====== CONFIG (Editable Part) ======
CORRECT_USERNAME = "Hi"
CORRECT_PASSWORD = "hellow"
CORRECT_REDEEM = "Qqq"
...
def loading(text):
    for i in range(30):
        dots = "." * (i % 4)
        sys.stdout.write(f"\r{YELLOW}{text}{dots}{RESET}")
        sys.stdout.flush()
        time.sleep(0.1)
    print()
...
def main():
    clear()
    logo()
    print(f"{CYAN}Checking Activation Status...{RESET}")
    loading("Verifying License")

    # simulate checking activation (you can add real logic here)
    time.sleep(1)
    activated = True  # change to False if you want to simulate not activated

    clear()
    logo()
    if activated:
        print(f"{GREEN}✔ STATUS: ACTIVATED 12:10 Pm End time {RESET}\n")
        loading("Logging in")
        clear()
        logo()
        print(f"{GREEN}✔ ACCESS GRANTED!{RESET}")
        print(f"{YELLOW}Welcome, Admin!{RESET}\n")
        ...
        print(f"{RED}✗ STATUS: NOT ACTIVATED{RESET}")
        print(f"{YELLOW}Please contact admin to activate your tool.{RESET}\n")
        sys.exit()
